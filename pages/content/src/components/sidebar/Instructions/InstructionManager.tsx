...
const InstructionManager: React.FC<InstructionManagerProps> = ({ adapter, tools }) => {
  // +++ LOGGING AÑADIDO +++
  console.log('[InstructionManager] Props received - Adapter instance:', adapter);
  console.log('[InstructionManager] Props received - Tools:', JSON.stringify(tools)); // Stringify para ver el contenido si es un array grande
  // +++ FIN LOGGING AÑADIDO +++
  ...
  useEffect(() => {
    // +++ LOGGING AÑADIDO +++
    console.log('[InstructionManager] useEffect [toolsSignature] triggered. Tools length:', tools.length);
    if (tools.length > 0) {
      ...
      logMessage('[InstructionManager] useEffect [toolsSignature]: No tools available to generate instructions.');
    }
    return () => {
  ...
  const handleInsertInChat = useCallback(async () => {
    if (!instructions) {
      logMessage('[InstructionManager] handleInsertInChat: No instructions to insert.');
      return;
    }
    if (!adapter || typeof adapter.insertTextIntoInput !== 'function') {
      logMessage('[InstructionManager] handleInsertInChat: Adapter or insertTextIntoInput method is not available.');
      console.error('[InstructionManager] Adapter or insertTextIntoInput method is not available:', adapter);
      return;
    }
    ...
    try {
      logMessage('[InstructionManager] handleInsertInChat: Calling adapter.insertTextIntoInput...');
      const success = await adapter.insertTextIntoInput(instructions);
      if (success) {
        setInsertSuccess(true);
        logMessage('[InstructionManager] handleInsertInChat: Insertion reported as successful by adapter.');
        setTimeout(() => setInsertSuccess(false), 2000);
      } else {
        logMessage('[InstructionManager] handleInsertInChat: Insertion reported as failed by adapter.');
      }
    } catch (error) {
      console.error('[InstructionManager] Error inserting instructions:', error);
      logMessage(`[InstructionManager] handleInsertInChat: Error - ${error}`);
    } finally {
  ...
  const handleAttachAsFile = useCallback(async () => {
    if (!instructions) {
        logMessage('[InstructionManager] handleAttachAsFile: No instructions to attach.');
        return;
    }
    if (!adapter || typeof adapter.supportsFileUpload !== 'function' || typeof adapter.attachFile !== 'function') {
        logMessage('[InstructionManager] handleAttachAsFile: Adapter does not support file upload or attachFile method is missing.');
        console.error('[InstructionManager] Adapter does not support file upload or attachFile method is missing:', adapter);
        return;
    }
    if (!adapter.supportsFileUpload()) {
        logMessage('[InstructionManager] handleAttachAsFile: Adapter reports file upload not supported.');
        return;
    }
  ...
  const handleCancel = useCallback(() => {
    const originalInstructions = instructionsState.listeners.length > 0 ? instructionsState.instructions : generateInstructions(tools);
          ...
          {isEditing ? (
            ...
            <>
              ...
              <ActionButton 
                ...
                disabled={!adapter || typeof adapter.supportsFileUpload !== 'function' || !adapter.supportsFileUpload() || typeof adapter.attachFile !== 'function'}
                color="purple" 
                label="Attach" 
              />
            </>
        ...
        {isEditing ? (
          ...
          <div className="max-h-64 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-600 scrollbar-track-transparent">
            <pre className="text-xs bg-slate-50 dark:bg-slate-800 p-3 rounded overflow-x-auto text-slate-700 dark:text-slate-300 whitespace-pre-wrap">
              {instructions || 'No instructions generated yet. Check console for tools prop.'}
            </pre>
          </div>
        )}
      </div>
